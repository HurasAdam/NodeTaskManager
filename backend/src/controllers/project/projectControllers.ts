  ...
  try {
    ...
    if (project) {
      return res.status(400).json({
        message:
          "Project with that name already exist. Project name must be unique.",
      });
    }
    ...
    const newProject = await Project.create({
      ...
      members,
      overview: [
        { header: "Project Objectives", content: target },
        { header: "Project Description", content: description },
      ],
      activities: activity,
    });
